...
wipe-test-db:
	RACKENV=test bundle exec rake db:wipe
...
webdriver-test:
	WEBDRIVER_TESTS=true RACK_ENV=development bundle exec rspec webdriver/$(file)
webdriver-test-staging:
	WEBDRIVER_TESTS=true MERGE_HEROKU_ENV=$(staging_app) bundle exec rspec webdriver/
webdriver-test-task:
...
restore-db-from-dump:
	@mkdir -p temp
	PGPASSWORD=webhookdb pg_restore --clean --no-acl --no-owner -h 127.0.0.1 -p 18005 -U webhookdb -d webhookdb temp/latest.dump || true
...
goto-sidekiq: cmd-exists-heroku
	open "https://`heroku config:get USERNAME --app=webhookdb-api-sidekiq-dash`:`heroku config:get PASSWORD --app=webhookdb-api-sidekiq-dash`@webhookdb-api-sidekiq-dash.herokuapp.com"
